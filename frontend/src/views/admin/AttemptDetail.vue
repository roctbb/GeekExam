  ...
  <div v-if="attempt" class="ge-fade-in">
    ...
    <div v-for="(q, i) in attempt.questions" :key="q.id" class="card mb-3">
      ...
      <div class="card-body">
        ...
        <div v-else class="text-muted small">{{ answer(q.id)?.check_state === 'checking' ? 'Проверяется...' : answer(q.id)?.check_comment || '' }}</div>
      </div>
    </div>
  </div>
...
async function grade(qid, aid) {
  ...
  if (attempt.value.answers.every(a => a.check_state === 'checked')) { attempt.value.total_points = attempt.value.answers.reduce((s, a) => s + (a.points || 0), 0); attempt.value.is_checked = true }
}
onMounted(async () => {
  const { data } = await api.getAttempt(route.params.id); attempt.value = data
  for (const q of data.questions) { const a = answer(q.id); grades[q.id] = { points: a?.points ?? 0, comment: a?.check_comment ?? '' } }
})
</script>
