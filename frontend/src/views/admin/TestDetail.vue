  ...
  <div v-if="test" class="ge-fade-in">
    <div class="ge-page-header">
      ...
      <div class="d-flex gap-2">
        <button v-if="!test.is_active" class="btn btn-success" :disabled="!test.code" @click="activate">🚀 Запустить</button>
        <button v-else class="btn btn-warning" @click="deactivate">⏸ Остановить</button>
        <RouterLink :to="`/admin/tests/${test.id}/results`" class="btn btn-outline-primary">📊 Результаты</RouterLink>
      </div>
    </div>
    ...
    <div v-for="v in test.variants" :key="v.id" class="mb-4">
      ...
      <div class="table-responsive">
        <table class="table table-sm table-bordered align-middle">
          <thead><tr><th style="width:50px">#</th><th>Вопрос</th><th>Тип</th><th>Проверка</th><th style="width:80px">Баллы</th></tr></thead>
          <tbody>
            <tr v-for="q in v.questions" :key="q.id">
              <td class="text-center">{{ q.order + 1 }}</td>
              <td>{{ q.title }}</td>
              <td><code>{{ q.type }}</code></td>
              <td><code>{{ q.check_type }}</code></td>
...
async function activate() {
  await api.activateTest(test.value.id)
  test.value.is_active = true
}
async function deactivate() {
  await api.deactivateTest(test.value.id)
  test.value.is_active = false
}
async function saveCode() {
  const { data } = await api.setTestCode(test.value.id, newCode.value)
  test.value.code = data.code
}
</script>
