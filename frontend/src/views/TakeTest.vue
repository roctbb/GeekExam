  ...
  <div v-if="store.attempt" class="ge-fade-in">
    ...
    <div class="ge-page-header">
      ...
      <div class="d-flex align-items-center gap-3">
        ...
        <button class="btn btn-danger" @click="confirmFinish">Завершить тест</button>
      </div>
    </div>
    ...
    <div v-if="currentQuestion" class="card mb-3">
      <div class="card-header d-flex justify-content-between align-items-center">
        <span class="fw-bold">{{ currentQuestion.title }}</span>
      ...
      <div class="card-body">
        ...
        <component
          ...
          :readonly="!!store.attempt.finished_at"
          :checkResult="currentAnswer"
          @update:modelValue="onAnswerUpdate"
          @check="onIntermediateCheck"
        />
        <div class="d-flex justify-content-between mt-3">
          <button class="btn btn-outline-secondary" :disabled="activeTab === 0" @click="activeTab--">← Назад</button>
          <button class="btn btn-outline-secondary" :disabled="activeTab === store.attempt.questions.length - 1" @click="activeTab++">Далее →</button>
        </div>
      </div>
    </div>
  </div>
  ...
  if (store.attempt.time_left !== null) {
    ...
    timerInterval = setInterval(() => {
      if (timeLeft.value > 0) {
        ...
        api.finishAttempt(route.params.id).then(() => router.push(`/my-results/${route.params.id}`))
      }
    }, 1000)
  }
