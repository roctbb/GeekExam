...
├── type            (String — тип вопроса: "text_input", "code_input", "true_false_table", "choice_table", "multi_input", "matrix_input", "interactive")
...
#### `matrix_input` — Квадратная матрица n × n

- **ui_config:** `{"size": 3}`; целое `size` от 1 до 50. Tab переходит к следующей ячейке; можно вставить таблицу с пробелами/табуляцией, двумерный JSON или плоскую последовательность. Вставка за пределы матрицы отклоняется.
- Необязательные `source_matrix`, `grayscale`, `kernel`, `kernel_divisor` показывают исходные данные рядом с ответом. Эталон в `ui_config` не помещается.
- Необязательные `fields: [{"name": "sum", "label": "Промежуточная сумма"}]` добавляют поля рядом с матрицей.
- **answer value:** `{"matrix": [["1", "2"], ["3", "4"]], "fields": {"sum": "10"}}` (пример для `size: 2`). Пустые ячейки — `""`, значения сохраняются строками.
- **check_config для exact:** `{"matrix": [[1, 2], [3, 4]], "scoring": "rows", "fields": {"sum": "10"}, "partial_scoring": true}`.
- `scoring`: `rows` (по умолчанию) — единица за полностью верную строку; `cells` — за ячейку; `all` — за всю матрицу. Дополнительное поле даёт одну единицу либо вес из `field_weights`. Балл равен округлённой доле верных единиц от `max_points`. При `partial_scoring: false` нужен полностью правильный ответ. Сравнение текстовое, с теми же опциями нормализации, что у остальных exact-вопросов.
- Импорт проверяет размер эталона, режим оценивания и соответствие дополнительных полей. Тип поддерживается при прохождении, в предпросмотре, редакторе и результатах ученика/преподавателя; миграция БД не нужна.

#### Необязательные варианты отображения

- `choice_table`: `ui_config.layout: "cards"` показывает отдельную карточку с подписанными radio для каждого элемента. Формат ответа и проверка прежние.
- `multi_input`: `layout: "table"`, `row_header`, `columns: ["Дисперсия"]`, `rows: [{"label": "T = 4", "fields": ["dispersion_1"]}]` группируют поля в таблицу. Остальные поля показываются ниже. Поле с `options: [{"value": "4", "label": "T = 4"}]` выводится как список выбора; значения вариантов — строки. Формат ответа остаётся объектом по именам полей.
- `code_input`: `layout: "split"` располагает условие и редактор рядом на широком экране; на узком — друг под другом. Примеры в условии можно копировать кнопкой.
- При отсутствии `layout` сохраняется прежнее отображение.

При проверке кода GeekExam формирует отчёт по `details` GeekPaste: имя из `tests[].name`, результат и причина ошибки, без скрытых входных данных и эталонного вывода. `callback_id` содержит идентификатор ответа и отпечаток проверяемой версии; запоздавший callback для изменённого ответа игнорируется. WebSocket передаёт проверенную версию для сравнения с ещё не сохранёнными правками в браузере. Старые callback ID остаются совместимыми; защита по версии применяется к новым заданиям на проверку.
...
3. Зарегистрировать тип в `backend/checkers/registry.py:VALID_QUESTION_TYPES` и списке типов редактора
4. Добавить поддержку результата, прогресса заполнения, валидации и нужного проверяющего обработчика
